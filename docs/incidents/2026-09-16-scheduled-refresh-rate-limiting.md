...
## Update: comparative spike evidence (2026-09-16, post-window)

A disposable GitHub Actions spike (see `docs/architecture/2026-09-16-refresh-platform-review.md`, Part 6, for full method and results) does not change the **root-cause classification** (still upstream rate limiting) but materially strengthens confidence in the "uncertainty" section above:

- At `2026-09-16T18:04:31Z`, an anonymous request from a GitHub-hosted runner to the identical `/launch/upcoming/` endpoint succeeded (`200`), with LL2's own `/api-throttle/` status showing that calling identity at only 1 of 15 hourly requests used.
- ~11 minutes later, at `2026-09-16T18:15:33Z`, LaunchCity's own natural Cloudflare cron cycle received `429` on the same endpoint (and again on its retry) - the **third** consecutive natural Cloudflare cycle observed to fail this way (previously `16:30:27` and `17:30:27`).
- A Cloudflare cycle at `18:00:42Z`, immediately before the `18:15` failure, **did succeed** - confirming the condition is intermittent, not a permanent block on LaunchCity's calling identity.

This raises confidence in the shared/fluctuating-identity hypothesis from "plausible, unproven" to "supported by direct comparative evidence" (still not conclusive - see the architecture document's confidence and remaining-uncertainty sections for the full caveats). Next step recommended there: investigate authenticated LL2 access (Option A) before any ingestion-path migration.
