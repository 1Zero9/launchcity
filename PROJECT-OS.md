...
| A "why this matters" or similar framing text on the recommended Launch Timeline product model could drift from factual/sourced into LaunchCity-authored editorial commentary | Would quietly turn LaunchCity into the general space-news site it has been explicitly told not to become | Not yet resolved — flagged as Decision Required in the product-direction output: if included at all, any such text must be strictly derived from LL2's own `mission.description`, never independently authored | — |
| A "Mission Control Board" or "Live-Event Companion" product framing would imply second-accurate live status, which the already-frozen fixed ~15 min, non-proximity-aware refresh cadence cannot honestly deliver | Building experience architecture around an urgency/live promise the backend can't keep would either mislead users or force reopening the frozen architecture | Recommended product direction (Launch Timeline anchored by Next Launch) deliberately avoids this framing — calm/honest personality chosen specifically because it doesn't require architecture changes to be true | — |
