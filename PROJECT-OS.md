...
- **[2026-09-14] Primary Timeline Experience implemented and FROZEN** — "The Horizon" visual direction, real cached LaunchCity data, responsive (desktop/tablet/mobile, verified with real browser screenshots), zero raster imagery. Frozen: next launch dominant; one continuous past→next→future chronology; launches visually attached to the horizon via event markers on a shared rail; NEXT the strongest point; surrounding launches visually recede; desktop horizontal horizon; mobile side-mounted vertical rail; photography not structurally required; real uncertainty visible and honest; no dashboard/card/navigation expansion.
- **[2026-09-15] Horizon Visual Implementation — FREEZE (superseding the above's deferred polish note)** — a visual fidelity review against the actual approved concept render found HIGH drift from the visual characteristics that led to The Horizon's selection (flat divider line instead of a curved horizon; no atmospheric depth; warm-only accent instead of a cool atmospheric signature; NEXT not physically connected to its marker) despite full compliance with every frozen structural/product rule above. A scoped, two-pass visual-only recovery restored: a shallow native CSS/SVG curved horizon; a cool cyan/blue atmospheric horizon signature kept separate from the warm accent (reserved for temporal/status meaning only); a light connector physically anchoring NEXT to its horizon marker; secondary events visually receding along the curve (not just positioned above it); sufficient width for real dominant-launch names; and a fixed mobile atmospheric-gradient boundary (previously noted as deferred polish - root cause was `radial-gradient`'s default `farthest-corner` sizing, not a spacing/typography nitpick). Decision: FREEZE HORIZON VISUAL IMPLEMENTATION as the accepted production interpretation of the approved direction - not required to reproduce the concept render pixel-for-pixel. Product Direction and Experience Architecture were not reopened. See Experiment Log and Learning Record, 2026-09-15.
...
**Date:** 2026-09-15
**What happened:** During a Cloudflare runtime verification, `wrangler kv key get`/`key list` (run without `--remote`) returned a full 50-launch snapshot with a plausible `lastSuccessfulRefresh` timestamp for the production KV namespace ID. This was treated as proof that the deployed `scheduled()` handler had successfully written to production, and used to justify a code fix (removing `getCacheStore()`'s module-scope memoization, since the HTTP path appeared to be silently masking that data with an empty `FileCache` fallback). The fix was implemented, tested, committed, and deployed. Only afterward, when the deployed fix still showed production as empty, was it discovered that `wrangler kv key get`/`key list` **default to local Miniflare-persisted state** (`.wrangler/state/v3/kv/`) unless `--remote` is explicitly passed - the "production" snapshot read was local dev/test data left over on this machine, not the real Cloudflare-hosted namespace. Re-run with `--remote`: `key get` → 404, `key list` → `[]`. The real production KV has never had a `launches` key.
**SUPERSEDES / MARKS INCORRECT:** The 2026-09-14T20:20:57Z-era claim (recorded only in-conversation, not previously committed to this file) that "production KV contained a valid last-good snapshot ... the HTTP application path was not consuming it" is **incorrect evidence** and is withdrawn. `/diagnostics` reporting empty/never/0 was accurate the whole time; there was no masked production data.
**What we learned:** A local Wrangler CLI read that appears to confirm production state can silently be reading Miniflare's local-persisted store instead - `wrangler kv key get`/`key list` without `--remote` is not evidence about the deployed Cloudflare resource. The code change made as a result (`lib/cache/index.ts`, commit `47955d4`) is independently justified - removing a fragile once-per-isolate memoization with a silent fallback is a real hardening - but it did not fix the originally-reported symptom, because the symptom was never caused by the defect it fixed.
**Reusable beyond this project?** Yes
**Candidate type:** Pattern
**Future relevance:** Operational evidence must identify whether it represents local emulation or the deployed environment. Verify remote platform state with the tool's explicit "remote" mode (or the raw platform API) before treating a local CLI read as production proof - especially before using it to justify a code change. Recorded as evidence only - not promoted into a Project OS rule.

### Learning record

**Date:** 2026-09-15
**What happened:** Enabled Cloudflare Workers Logs (Observability) on the deployed `launchcity` script via a metadata-only API call (no redeploy, no new Worker version) and, using `wrangler tail`, directly observed one natural (not manually triggered) `*/15 * * * *` Cron invocation end-to-end. Captured evidence: `scheduled()` entered and ran `refreshLaunchData()`; `fetchUpcoming()` was attempted and received `LL2 request failed: 429 Too Many Requests (/launch/upcoming/?limit=30&mode=detailed)`; `fetchPrevious()` was never attempted (the code awaits `fetchUpcoming()` first and the shared `try` throws on its failure); the refresh returned `ok:false` and, by design, never called `store.write()` - logged verbatim as `"[scheduled refresh] failed, previous cache snapshot preserved"`; the Worker's own top-level `outcome` was still `"ok"` (the handler caught its own error and returned normally - a failed refresh is not a Worker-level failure). Separately confirmed via `--remote`: the real production KV **does** now hold a `launches` key with real data (50 launches, `lastSuccessfulRefresh: 2026-09-15T10:30:20.176Z`, `requestCost: 2`) - written by an earlier, different, successful cron cycle (the observed 11:00 invocation did not write; a prior ~10:30 invocation did).
**What we learned:** The scheduled-refresh architecture works end-to-end against real Cloudflare + real LL2 when LL2 doesn't reject the request - directly proven, not inferred, by the 10:30 successful write. The specific invocation observed live failed for a single, unambiguous, captured reason: LL2's free-tier rate limit (429), matching the failure mode already documented in the 2026-09-14 Learning Record above. This is now first-hand evidence from the actual deployed production Worker, superseding that record's reliance on general/historical observation.
**Reusable beyond this project?** Yes
**Candidate type:** Pattern
**Future relevance:** When a scheduled/background job's failure is intermittent (works sometimes, fails other times) rather than constant, don't stop at "it's currently failing" - check whether the underlying path has ever succeeded for real (here: a real remote KV write with a real timestamp) before concluding the architecture itself is broken. Not promoted into a Project OS rule.

### Learning record

**Date:** 2026-09-15
**What happened:** A visual fidelity review compared the approved "Horizon" concept render (the actual attached image, not a text description of it) against the live implementation. Every frozen structural/product rule held exactly (2 surfaces, no nav/cards/tabs/countdown, honest uncertainty, progressive disclosure) - but the concept's defining visual signature had been lost: a flat CSS gradient line instead of a curved horizon, zero atmospheric depth, a warm accent instead of the concept's cool cyan/blue atmospheric identity, and no visual fusion between NEXT and its horizon marker. Classified as HIGH drift, requiring TARGETED VISUAL RECOVERY - explicitly not a redesign, and Product Direction/Experience Architecture were never reopened. A scoped, two-pass recovery followed: (1) a native CSS/SVG curved horizon, cool-signature atmospheric glow, a NEXT-to-horizon light connector, and separated warm-for-status/cool-for-atmosphere colour roles; (2) corrections from a second real-screenshot review - wider dominant block, a longer/stronger NEXT connector, a faint distance-scaled connector on secondary items so they read as receding along the curve rather than as equal text columns, and a mobile atmospheric-glow fix (the visible rectangular boundary was `radial-gradient(ellipse ...)`'s default `farthest-corner` sizing never reaching true transparent at the box's straight edges - fixed with `closest-side` sizing). Both passes validated against real cached data (long/wrapped names, "Unknown Payload", varying provider/vehicle/site lengths) with the full test/typecheck/lint/build suite, never by optimising around the concept's short mock names. Decision: FREEZE HORIZON VISUAL IMPLEMENTATION - accepted as the production interpretation of the approved direction, not a pixel-for-pixel reproduction of the render.
**What we learned:** *"A frozen visual direction can remain structurally compliant while losing the visual characteristics that caused the direction to be selected."* Confirmed directly: compliance with frozen product/experience rules and fidelity to the perceptual identity that earned a direction its approval are separate axes - passing one gives no signal about the other, and only an actual side-by-side comparison against the original evidence (not the text description of a decision) surfaces the gap. Separately: targeted visual recovery restored that fidelity without reopening Product Direction or Experience Architecture - a visual-fidelity gap did not require, and should not automatically trigger, redesign at a higher layer than where the drift actually occurred.
**Reusable beyond this project?** Yes
**Candidate type:** Pattern
**Future relevance:** When reviewing a frozen visual/creative direction for drift, compare against the actual original artifact, not a memory or text summary of it, before concluding fidelity holds or has failed. Not promoted into a Project OS rule.
...
| 2026-09-15 | Production empty-cache investigation — Cloudflare runtime verification | Determine whether production's empty `/diagnostics` state was a Cloudflare platform config defect or an application code defect | Yes | Low | Verified Worker, Cron Trigger, KV binding, and secrets all present and correctly configured in the deployed Cloudflare state (not just `wrangler.jsonc`). A local (non-`--remote`) `wrangler kv key get` read was mistakenly treated as production evidence of a valid cached snapshot - **later corrected as incorrect, see Learning Record above**. |
...
## 7. Founder Correction — 2026-09-16

**Status: FOUNDER DECISION — ACTIVE.** This section records a direct founder correction, made after the read-only Original Intent and Product Drift Review, and is not an assistant-inferred interpretation, a Scope & Stop check, or a new experiment. Full detail: `docs/product/2026-09-16-founder-product-direction.md`.

**Image-free LaunchCity is SUPERSEDED.** §1's "LaunchCity v0.1 Visual Direction — APPROVED: THE HORIZON" entry and its "confirmed: 0 `<img>` tags" note (2026-09-14), and the "FREEZE HORIZON VISUAL IMPLEMENTATION" entry (2026-09-15), each recorded the zero-image state as settled fact. Steve has now confirmed that state originated as a temporary visual comparison, not a permanent decision, and that he never approved permanent image exclusion. Those historical entries are **preserved below and in their original locations, unaltered** — they remain accurate evidence of how a temporary experiment hardened into undocumented permanent direction without a founder-approval event, which is itself the material finding this correction responds to.

**New status: imagery is PROVISIONAL, pending visual proof.**
- The Horizon's dominant next launch and Launch Detail may use a real, appropriately-licensed image, with graceful, honest fallback to the existing atmospheric Horizon treatment when no suitable image exists.
- Secondary Horizon items remain image-free for now (also PROVISIONAL, to preserve Horizon clarity during the proof stage).
- No image implementation has been built or deployed as a result of this entry. This is a direction record, not a completed change.

**LaunchCity's purpose is recorded as dual and ACCEPTED:** a useful public rocket-launch product for general space enthusiasts, and a 1Zero9 portfolio showcase. Both halves are required simultaneously; neither purpose alone is sufficient.

**Freshness position ACCEPTED:** ~15 minutes is the normal target; ~30 minutes is an acceptable occasional tolerance; multi-hour staleness is REJECTED as unacceptable. Exact cadence precision is an implementation choice, not a product requirement — this loosens, rather than tightens, the assumption previously recorded in §1 Architecture item 2.

**Cost position ACCEPTED:** LaunchCity remains free or very low recurring cost while it earns no revenue. No paid service is approved by this entry.

**PAUSED, unchanged by this correction:** authenticated Launch Library 2 access (pending a separate founder cost decision) and further platform migration away from or within Cloudflare (pending evidence of necessity). The 2026-09-16 architecture review (`docs/architecture/2026-09-16-refresh-platform-review.md`) and its open questions remain as recorded; this entry does not resolve them.

**FROZEN, unchanged and reaffirmed:** the zero-fabrication and honest-uncertainty policy. Nothing in this correction weakens it — imagery fallback rules explicitly extend it ("do not fabricate mission artwork," "do not silently substitute an unrelated launch image").

**Scope note:** this correction authorises a bounded visual-proof stage for imagery (see the completion sequence in `docs/product/2026-09-16-founder-product-direction.md`), not search, accounts, notifications, a new database, a new hosting platform, a paid subscription, or a full Horizon redesign. The two-surface structure (Horizon, Launch Detail) remains the accepted completion-stage boundary, not a permanent ceiling on future versions.

**Why prior entries are not rewritten:** §1's Horizon/imagery entries and the Experiment Log rows above accurately describe what was built and frozen at the time, under the process running at the time. Editing them would erase the evidence that a temporary comparison hardened into undocumented permanent direction — exactly the failure this correction exists to name and fix. They stand as historical record; this section is the current, active direction.
